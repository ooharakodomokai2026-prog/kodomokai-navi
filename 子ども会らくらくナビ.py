...
* 迎える会の準備: 大原会館の予約は管理者の山口さんへ用紙提出（当日はカギを受け取る）。新1年生へのプレゼント（約1,000円。例年「名入り鉛筆」「折りたたみ傘」が好評！）や、全員用のお菓子を手配しておきます。""", height=160, key="knowhow_4")
    ...
    st.subheader("💡 ノウハウ＆アドバイス（※画面上で直接編集可能）")
    st.text_area("編集可能なアドバイス欄", value="""* 迎える会当日のプログラム: あいさつ ➔ 自己紹介 ➔ ゲーム ➔ おわりのあいさつ ➔ おみやげ（解散）。
* 雨天時の対応: 雨なら「会館内でゲーム」（※倉庫に道具あり）。過去には「BBQ」の提案もありました。
* 球技大会案内: 青崎小、新町小の両校に通う町内の子どもたちにも届くよう、地域掲示板6ヶ所にポスターを掲示して子ども会のグループLINEでお知らせします。""", height=160, key="knowhow_5")
...
* 備品手配: 金魚すくい等の「たらい」は町内会から借ります。ダンボールで看板を用意しましょう。""", height=140, key="knowhow_7")
    ...
    st.subheader("💡 ノウハウ＆アドバイス（※画面上で直接編集可能）")
    st.text_area("編集可能なアドバイス欄", value="""* ラジオ体操最終日: 頑張った子どもたちへアイスクリームやお菓子を用意してね！（歓迎会の時にまとめて購入して配れるようにしておくと楽です）
* 盆踊りのお店は帰省者が多いので出店するなら１店舗。（おすすめはかたぬき屋さん）
* 夏フェス: 16:45集合。17:00〜17:30までは大人対応してもらい、子どもには先に遊んでもらいます。17:30～18:00 / 18:00〜18:30は子どもお手伝い参加。19:00までの残りは大人対応。大人は1時間交代がおすすめ。寄付金等をいただいた場合は後日お礼（お返し）を用意し、9月定例会で収支報告をします。""", height=200, key="knowhow_8")
...
* 運動会のお弁当: 当日手伝ってくれる中学生や保護者の人数もカウントして、お弁当やお茶をしっかり発注しておきましょう。""", height=140, key="knowhow_9")
    ...
    st.subheader("💡 ノウハウ＆アドバイス（※画面上で直接編集可能）")
    st.text_area("編集可能なアドバイス欄", value="""* お菓子手配: 秋祭りのお菓子は人数分より多めに用意。10日前までに注文を完了させます。
