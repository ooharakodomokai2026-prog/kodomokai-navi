...
* **迎える会の準備:** 大原会館の予約は管理者の山口さんへ用紙提出（当日はカギを受け取る）。新1年生へのプレゼント（約1,000円。例年「名入り鉛筆」「折りたたみ傘」が好評！）や、全員用のお菓子を手配しておきます。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
* **迎える会当日のプログラム:** あいさつ ➔ 自己紹介 ➔ ゲーム ➔ おわりのあいさつ ➔ おみやげ（解散）。
...
* **備品手配:** 金魚すくい等の「たらい」は町内会から借ります。ダンボールで看板を用意しましょう。""")
    ...
    with col1:
        ...
        st.checkbox("夏フェス当日の準備（16:45集合、机・イス運搬）・運営・片付け")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
* **ラジオ体操最終日:** 頑張った子どもたちへアイスクリームやお菓子を用意してね！（歓迎会の時にまとめて購入して配れるようにしておくと楽です）
...
* **運動会のお弁当:** 当日手伝ってくれる中学生や保護者の人数もカウントして、お弁当やお茶をしっかり発注しておきましょう。""")
    ...
    with col1:
        ...
        st.checkbox("秋祭り用お菓子の手配（人数分より多めに用意！10日前までに注文）")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
* **お菓子手配:** 秋祭りのお菓子は人数分より多めに用意。10日前までに注文を完了させます。
...
* **引き継ぎ:** この「らくらくナビ」自体が引き継ぎマニュアルです！""")
    ...
    st.write("「6年生を送る会」の実施と、大原町子供会総会を開催し、1年間の締めくくりと新年度役員へ引き継ぎを行います。")
    col1, col2 = st.columns([3, 2])
    with col1:
        st.subheader("📝 今月の「やる事」リスト")
