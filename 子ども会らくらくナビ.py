...
if selected_month == "4月":
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **新1年生案内:** 小学校からの情報提供がないため、町内会の総書さんへ「新1年生の回覧が集まったら教えてほしい」と事前にお願いしておくのがコツ！
    * **ポスター掲示:** 町内会未加入の方に向けて、地域の掲示板（6ヶ所）に強力テープでポスターを貼り出します。
    * **迎える会の準備:** 大原会館の予約は山田さんへTELして用紙提出（当日はカギを受け取る）。新1年生へのプレゼント（約1,000円。例年「折りたたみ傘」が好評！）や、全員用のお菓子を手配しておきます。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **迎える会当日のプログラム:** あいさつ ➔ 自己紹介ゲーム ➔ ころがしドッチ ➔ おわりのあいさつ ➔ おみやげ（解散）。
    * **雨天時の対応:** 雨なら「会館内でゲーム」、晴れなら「公園でドッヂボール」（※倉庫に道具あり）。過去には「BBQ」の提案もありました。
    * **球技大会案内:** 他校（新田小など）に通う町内の子どもたちにも届くよう、地域掲示板6ヶ所にポスターを掲示してLINEでお知らせします。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **アンケート収集:** 射的（50円）やフランクフルト、ボールすくいなど、高学年の子どもたちが中心となって出店できるようにアンケートを取って企画をまとめます。
    * **チケット作業:** 50円チケットは量が多いので、役員同士で手分けしてハサミで切っておくと後が楽です！""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **ラジオ体操:** 当日はYouTubeでラジオ体操を流します（スピーカーは会館にもあり）。雨天中止の判断は6:00〜6:15頃に子ども会LINEで連絡します。
    * **備品手配:** 金魚すくい等の「たらい」は町内会から借ります。ダンボールで看板を用意しましょう。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **ラジオ体操最終日:** 頑張った子どもたちへアイスクリームを用意してね！
    * **夏フェス本番:** 17:30集合。18:00〜19:00 / 19:00〜20:00の2交代制。高学年の店番に大人の見守り係を配置します。手ふき等をいただいた場合は後日お礼（お返し）を用意し、9月定例会で収支報告をします。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **御寄付のお礼:** 御寄付をいただいた方には、必ず後日「菓子折り」と「お礼状」をお渡しするのがマナーです！
    * **運動会のお弁当:** 当日手伝ってくれる中学生や保護者の人数もカウントして、お弁当やお茶をしっかり発注しておきましょう。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **お菓子手配:** 秋祭りのお菓子は60袋用意。10日前までに注文を完了させます。
    ...
    * **映画鑑賞会:** 会館のプロジェクターを使うため、早めに管理人の山岡さんへ予約をお願いしておきます。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **三世代交流:** 育成から連絡が来ます。お昼のお弁当（うどん等）は育成側で用意してくれます。
    * **クリスマス会準備:** 子どもたちにサンタ帽子作りなどの工作をさせる場合は、11月中に役員で試作しておきます。ケーキも早めに予約しましょう。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **クリスマス会当日の流れ:** ボードゲーム ➔ サンタ帽子工作 ➔ ケーキデコレーション＆実食 ➔ サンタ＆トナカイからプレゼント登場！ ➔ 片付け。""")
    ...
    st.info("""💡 **前会長の神ノウハウ＆アドバイス**
    * **6年生の記念品:** 予算は3,000円程度（例: QUOカードなど）。1月中に6年生本人に希望を聞いて手配しておきます。
